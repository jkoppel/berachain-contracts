...
contract GovDeployer is Create2Deployer {
    ...
    uint256 internal immutable AVG_BLOCK_TIME_MS;
    ...
    /// @param avgBlockTimeMs The average block time in milli-seconds
    ...
    constructor(
        ...
        AVG_BLOCK_TIME_MS = avgBlockTimeMs;
        ...
        InitialGovernorParameters memory params = InitialGovernorParameters({
            ...
            votingDelay: uint48(timeToBlocks(votingDelay)),
            votingPeriod: uint32(timeToBlocks(votingPeriod))
        });

        governance.initialize(IVotes(token), TimelockControllerUpgradeable(timelock), params);

        // As this contract uses `timeToBlocks`, check that the clock mode is the expected one:
        require(
     ...
     */
    function timeToBlocks(uint256 time) public view returns (uint256 blocks) {
